...
function ApiDemoSection() {
  ...
  const handleSendRequest = async () => {
    ...
    // Authenticated - redirect to API playground
    ...
    <section className="px-4 py-20 md:px-8 border-b border-border">
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          {/* Modal */}
          <div className="relative bg-white border border-gray-200 shadow-2xl p-8 max-w-md w-full mx-4 rounded-2xl animate-in fade-in zoom-in duration-200">
            {/* Close button */}
            <button
              onClick={() => setShowAuthModal(false)}
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">
                Sign in with your Google account to use the API playground and test the GitHub Summarizer.
              </p>

              {/* Sign in button */}
              <button
                onClick={() => signIn("google", { callbackUrl: "/dashboards/playground" })}
                className="flex items-center justify-center gap-3 w-full h-12 border-2 border-gray-200 bg-white hover:bg-gray-50 px-5 text-sm font-semibold text-gray-900 shadow-sm transition-all hover:shadow-md rounded-xl"
              >
              ...
              <p className="mt-4 text-xs text-gray-500">
                You'll be redirected to the API Playground after signing in.
              </p>
            </div>
          </div>
        </div>
